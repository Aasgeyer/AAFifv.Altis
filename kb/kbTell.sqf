...
_topicName =    _this param [0,"",[""]];
_mission =    _this param [1,missionname,[""]];
_section =    _this param [2,"",[[],""]];
_channel =    _this param [3,false,[false,"",0,{},objnull]];
_sentenceCode =    _this param [4,{true},[{},[]]];
_listUnits =    _this param [5,[],[[],true]];
_volumeCoef =    _this param [6,-1,[1]];
_disableRadio =    _this param [7,true,[true]];
_exit = false;
_sectionVariant = "";

//check for code params
_sentenceCodeParams = _sentenceCode param [1,[],[[]]];
_sentenceCode = _sentenceCode param [0,{true},[{}]];
...
//--- Get conversation ID
private _topicIDValue = getnumber (_cfgTopic >> "id");
private _topicID = if (_topicIDValue > 0) then {(((_topicIDValue / 1000) tofixed 3) + "_") select [2]} else {""}; //--- Result in format "000"
...
//--- Get units
_createDummy = if (!isnil "bis_fnc_kbTell_createDummy") then {bis_fnc_kbTell_createDummy param [0,false,[false]]} else {false};
...
if (count _listUnits > 0) then {
    ...
    _sentenceStart =    _section param [0,_sentenceStartDefault,["",[],true]];
    _sentenceEnd =        _section param [1,_sentenceEndDefault,["",[],true]];
    ...
    [["conversationStart",[_volumeCoef, _disableRadio]],"BIS_fnc_kbTellLocal",(_listUnits + playableUnits + switchableUnits)] call bis_fnc_mp;
    ...
    for "_s" from _sentenceStartId to _sentenceEndId do {
        ...
        //--- Unless we are running internal version
        private _acctime = acctime;
        if (!cheatsEnabled) then {setacctime 1};
        ...
        //--- Speak!
        [["sentence",[_from,_params],[_mission,_topicName]],"BIS_fnc_kbTellLocal"] call bis_fnc_mp;
...
*/
        ...
        if (!cheatsEnabled) then {setacctime _acctime};
    ...
    [["conversationEnd",[_volumeCoef, _disableRadio]],"BIS_fnc_kbTellLocal",(_listUnits + playableUnits + switchableUnits)] call bis_fnc_mp;
